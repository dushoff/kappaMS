...
\ag{Can we use the Introduction section available on Overleaf?}
...
In particular, we proved that the variance in case-per-case, conditioned on the entire outbreak, is equal to one, regardless of the key parameter \Ro~(Section \ref{sec_proof}).
\begin{figure}
\centering
...
proof?} \label{sec_proof}
\ag{May I place Tapan's proof here?}
\section{Discussion}
