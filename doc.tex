...
A large \Ro~ initially generates many secondary cases. Later, rapid depletion of susceptible individuals reduces the chance of large secondary cases.
...
We then split the total variance in secondary-case over the course of the outbreak into between- and within-cohort components. We found that epidemics with larger \Ro\ have larger between-cohort variation, as expected. However, this is balanced by smaller within-cohort variation (\Fref{RcAverage}, lower panel). 
...
\textbf{Indistinguishable empirical secondary-case distributions  for epidemics of different strengths conditioned over the entire outbreak cycle.}
	...
	Identical variance in secondary cases for epidemics of different strengths modeled using simple compartmental models (lower panel).
...
At each time point, we calculated heterogeneity in secondary cases based on the cases generated up to that time by infected cohorts.
...
By the end of the outbreak, the heterogeneity in secondary cases for different values of \Ro~became indistinguishable and equal to one.

We also examined an idealized scenario in which, for each time point, we computed heterogeneity in secondary cases over the entire outbreak, considering all cohorts that had become infectious up to that time.
In epidemics with lower \Ro, the mean and variance of secondary cases across early-infected cohorts were the same as those in a linearized model (\fref{timeCutoff}).
...
By the end of the outbreak, all epidemics with different \Ro~agreed on the heterogeneity in secondary cases, measured by $\kappa$.
...
\begin{figure}[H]
	...
	\caption{
		\textbf{The secondary-case distribution evolves over the course of outbreak.}
		...
		Panel c: The squared coefficient of variation of secondary-cases $\kappa$ climbs over the course of the outbreak.
...
\begin{figure}[H]
	...
	\caption{
		\textbf{Forecasting the secondary-case distribution evolves as the outbreak unfolds.}
		...
		The mean of secondary-cases is larger in early-infected cohorts and depends on the key parameter \Ro. 
		As time goes by, later-infected cohorts have larger weights, as measured by incidence.
		These cohorts experience susceptible pool depletion, leading to fewer secondary cases. 
