...
The heterogeneity in secondary cases shown in \Fref{ls} does not consider the depletion of the susceptible population. How would the secondary distribution change if we incorporated changes in the susceptible pool, as occurs in real outbreaks?
...
This result is unexpected. Significantly greater dynamical changes occur when \Ro~is large, which would typically be expected to correspond to greater variation in the secondary case distribution (\Fref{SIRsim}).
...
A large \Ro~initially generates many secondary cases \ag{What about changing ``secondary cases'' to individual reproductive number or expected infectiousness in deterministic models?}. Later, rapid depletion of susceptible individuals reduces the chance of large secondary cases.
In contrast, a small \Ro~produces fewer initial secondary cases, but the slower susceptible exhaustion allows for larger secondary cases (\Fref{Cohort}).
We then split the total variance in secondary-case over the course of the outbreak into between- and within-cohort components. We found that epidemics with larger \Ro\ have larger between-cohort variation, as expected. However, this is balanced by smaller within-cohort variation (\Fref{RcAverage}, panel b). 
We can in fact prove that the variance in secondary cases, conditioned on the entire outbreak, is equal to one, regardless of the value of \Ro~(Section \ref{sec_proof}).
\begin{figure}
\centering
...
(b)	Identical variance in secondary cases \ag{or individual reproductive number? or expected infectiousness?} for epidemics of different strengths modeled using simple compartmental models.
...
We are also interested in how secondary-case distributions appear to researchers analyzing outbreaks in real time.
...
At each cutoff point, we calculated heterogeneity in secondary cases based on cases generated by infected cohorts up to that time.
...
By the end of the outbreak, heterogeneity in secondary cases for different \Ro~ values became indistinguishable.

We also examined an idealized scenario in which, for each cutoff point, we computed heterogeneity in secondary cases over the entire outbreak, considering all cohorts that had become infectious up to that time.
...
\begin{figure}[p]
	...
	\caption{
		\textbf{The distribution of secondary cases evolves over the course of outbreak.}
		...
		Panel b: The mean and standard deviation of secondary cases at each time are computed using the realized cases up to that point. 
...
\begin{figure}[H]
	...
	\caption{
		\textbf{Forecasting the distribution of secondary cases evolves as the outbreak unfolds.}
		...
		Panel b: At each time point, the mean and standard deviation of the secondary cases are calculated using only cohorts infected up to that time, while accounting for their subsequent roles in disease transmission.
		...
		The mean of secondary cases is larger for early-infected cohorts and depends on the key parameter \Ro. 
		As time goes by, later-infected cohorts, who have larger sizes,
		experience susceptible pool depletion, leading to fewer secondary cases. 
