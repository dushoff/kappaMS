...
Let $f$ denote the probability density function of the infectious period. Define $F(t)$ as the corresponding survival distribution, that is
...
%                         Finish here
% --------------------------------------------------------------
\subsection*{Incorporating the Contribution of the Initial Infections}
 Mathematical derivation in the previous section was based on the assumption that the initial infectious cohort was negligible.
 In what follows, we revisit the results while relaxing this assumption.
Let $y_0$ denote the proportion of the population that is infected at time $0$.
 The contribution of these initial infections can be incorporated into the expressions above by changing the lower limit of integrations to $0^-$ and taking $i(t)=y_0\delta(t)$ for $t\in(-\infty,0]$, where $\delta(t)$ denotes the Dirac Delta function. Then, the integral

$$
\int_{0^-}^t d\tau\; i(\tau)F(t-\tau)
$$
remains equal to $y(t)$. 
The corresponding equation for the first moment $C_1$ which now also accounts for the initial infections, remains unchanged, provided that the initial infections are not accumulated in $Z$, i.e., final size doesn't include the initial infections.

The corresponding expression for the second moment $C_2$, which now also accounts for the initial infections, changes to
\begin{equation} \label{eq:C2withI0}
	\begin{aligned} 
		C_2 = 2Z - 2y_0 \Ro \int_0^\infty dt\;x(t)e^{-t}
	\end{aligned}
\end{equation}
The second term in the right-hand side of \autoref{eq:C2withI0} is twice the expected infectiousness of the initial infections. 
\newpage
