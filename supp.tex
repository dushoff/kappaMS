...
Let $y_0$ denote the proportion of the population that is infected at time $0$.
 The contribution of these initial infections can be incorporated into the expressions above by changing the lower limit of integration to $0^-$ and taking $i(t)=y_0\delta(t)$ for $t\in(-\infty,0]$. Then,

$$
\int_{0^-}^t d\tau\; i(\tau)F(t-\tau)
$$
remains equal to $y(t)$. 
The corresponding equation for the first moment $C_1$ which now also accounts for the initial infections, remains unchanged, provided that the initial infections are not accumulated in $Z$.
...
\begin{figure}[H]
	...
	\caption{
		...
		In Panel b, the mean and standard deviation of expected infectiousness associated with each cohort are plotted.	
		%	In an
